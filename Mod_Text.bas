...
x1 = Abs(IsLabel(basestack, rest$, pa$, , s$))
...
If x1 <> 0 Then
        If SubName$ = "" Or pa$ = "" Then MyEr "Nothing to save", "Δεν υπάρχει κάτι να σώσω":                Exit Function
        ...
        Do While IsString(basestack, w$, s$)
                ...
                If Lang Then
                        ...
                        Else
                                If Not blockCheck(SB(col), DialogLang, "Συνάρτηση " & Left$(s$, InStr(s$, " ") - 1) + "()" + vbCrLf) Then Exit Function
                                prg$ = "ΣΥΝΑΡΤΗΣΗ " & Left$(s$, InStr(s$, " ") - 1) & " {" & SB(col) & "}" & vbCrLf + prg$
                        End If
                Else
                        col = Val(Mid$(s$, InStr(s$, " ") + 1))
                        If Right$(SB(col), 2) <> vbCrLf Then SB(col) = SB(col) + vbCrLf
                        ...
                        Else
                                If Not blockCheck(SB(col), DialogLang, "Τμήμα " & Left$(s$, InStr(s$, " ") - 1) + vbCrLf) Then Exit Function
                                prg$ = "ΤΜΗΜΑ " & Left$(s$, InStr(s$, " ") - 1) & " {" & SB(col) & "}" & vbCrLf + prg$
                        End If
                End If
        Loop
        w$ = ""
...
Do
    ...
    ElseIf FastSymbol(aa$, "/") Then
            ...
            Else
                ...
                Exit Function
            End If
    ElseIf IsLabelSymbolLatin(aa$, "DIV") Or IsLabelSymbol(aa$, "ΔΙΑ") Then
            If logical(bstack, aa$, r) Then
                MUL = 4
                R1 = po
                po = r
            ...
            Else
                ...
                Exit Function
            End If
ElseIf IsLabelSymbolLatin(aa$, "MOD") Or IsLabelSymbol(aa$, "ΥΠΟΛΟΙΠΟ") Or IsLabelSymbol(aa$, "ΥΠΟΛ") Then
            If logical(bstack, aa$, r) Then
                MUL = 5
                R1 = po
                po = r
...
ElseIf FastSymbol(aa$, ")") Then
            ...
            Else
                ...
                Exit Function
            End If
ElseIf IsLabelSymbolLatin(aa$, "XOR", , Not noand) Or IsLabelSymbol(aa$, "ΑΠΟ", , Not noand) Then
'  good
  MUL = 3
If FastSymbol(aa$, "(") Then
    If IsExp(bstack, aa$, r) Then
...
ElseIf IsLabelSymbolLatin(aa$, "OR", , Not noand) Or IsLabelSymbol(aa$, "Η", , Not noand) Then
'  good
  MUL = 3
If FastSymbol(aa$, "(") Then
    If IsExp(bstack, aa$, r) Then
...
ElseIf IsLabelSymbolLatin(aa$, "AND", , Not noand) Or IsLabelSymbol(aa$, "ΚΑΙ", , Not noand) Then
'  good
  MUL = 3
If FastSymbol(aa$, "(") Then
    If IsExp(bstack, aa$, r) Then
...
IsNumber = False  ''         " " + & String$(w2 - Len(n$), " ")
 ...
 MyErMacro A$, "Unkown Property " & V$, "Άγνωστη ιδιότητα " & V$
 Else
MyErMacro A$, "Unkown Variable " & V$, "Άγνωστη μεταβλητή " & V$
End If
End If
Exit Function
...
skiperror:
        A$ = n$: IsNumber = False
         If FindNameForGroup(bstack, V$) Then
 MyErMacro A$, "unknown method/array  " & V$, "Άγνωστη μέθοδος/πίνακας " & V$
 Else
     MyErMacro A$, "unknown function/array " & V$, "Άγνωστη συνάρτηση/πίνακας " & V$
...
End Function
Function IsLabel(bstack As basetask, A$, rrr$, Optional nocommand As Boolean, Optional r$) As Long
...
Case Is >= "A"
  ...
  ''                              ElseIf InStr(arrname$, Chr$(13) + HERE$ & "." & myUcase(Mid$(r$, 1, Len(r$))) + " ") > 0 Then
   ...
   End If
    ...
    IsLabel = RR&
    nocommand = IsLabel And (Len(r$) = 1 Or nocommand)
 '' a$ = LTrim(a$)
...
itisavar:
                ...
                Else
                     ...
                     If w2 >= Len(A$) Then A$ = q$ + A$
                             ...
                             MyErMacro A$, "Unkown property  " & q$, "Άγνωστη ιδιότητα " & q$
                             Else
                                MyErMacro A$, "Unkown variable " & q$, "Άγνωστη μεταβλητή " & q$
...
skiperrorStr:
         If FindNameForGroup(bstackstr, q$) Then
 MyErMacro A$, "unknown method/array  " & q$, "Άγνωστη μέθοδος/πίνακας " & q$
 Else
     MyErMacro A$, "unknown function/array " & q$, "Άγνωστη συνάρτηση/πίνακας " & q$
...
Else
'this cant find B16
IsLabelSymbol3 = IsLabelSYMB3(TEST$, what$)
End If
If what$ = "" Then
...
If Bypass Then Exit Function
  ...
  If A$ <> "" And c$ <> "" Then
TEST$ = A$
IsLabelSymbol = IsLabelSYMB3(TEST$, what$)
...
If Bypass Then Exit Function
  ...
  If A$ <> "" And c$ <> "" Then
TEST$ = A$
IsLabelSymbolLatin = IsLabelSYMB3(TEST$, what$)
...
PROCESSCOMMAND:
 ...
 MyEr "Unkown Property " & w$, "Άγνωστη ιδιότητα " & w$
 Else
MyEr "Unkown Variable " & w$, "Άγνωστη μεταβλητή " & w$
End If
...
If MaybeIsSymbol(b$, "=") Then
b$ = Mid$(Trim$(b$), 2)
If (InStr(HERE$, "(") > 1) Then   ' return from a function no error checking
    If IsExp(bstack, b$, p) Then
                        If Len(HERE$) > 3 Then
                                            If Mid$(HERE$, Len(HERE$) - 2, 1) = "$" Then
                                                MissStringExpr
                                                Exit Do
                                            End If
                                End If
                If Mid$(HERE$, Len(HERE$) - 2, 1) = "%" Then
                        bstack.FuncValue = Int(p)
              Else
                    ...
                    If Len(HERE$) > 3 Then
                        ...
                        Else
                                    MissNumExpr
                                        ...
                                        Exit Function
                        End If
'' checking for return Document object
     ''       bstack.FuncValue = ss$
                  Set bstack.FuncObj = bstack.LastObj
                              Set bstack.LastObj = Nothing
                                bstack.FuncValue = SS$
...
If IsLabelSymbolNew(b$, "ΑΥΤΟ", "THIS", Lang) Then
      If bstack.Process Is Nothing Then
                b$ = ""
                MyEr "Clause This can't used outside a thread", "Ο όρος AYTO δεν μπορεί να χρησιμοποιηθεί έξω από ένα νήμα"
                Execute = 0: Exit Function
                Once = False
            ElseIf IsLabelSymbolNew(b$, "ΣΒΗΣΕ", "ERASE", Lang) Then
         Set bstack.Process.Process = Nothing

                b$ = "" ' get lost now
            ElseIf IsLabelSymbolNew(b$, "ΚΡΑΤΑ", "HOLD", Lang) Then
                bstack.Process.busy = True

            ElseIf IsLabelSymbolNew(b$, "ΞΕΚΙΝΑ", "RESTART", Lang) Then
                bstack.Process.busy = False
                b$ = ""
            ElseIf IsLabelSymbolNew(b$, "ΚΑΘΕ", "INTERVAL", Lang) Then
                                 If IsExp(bstack, b$, p) Then
                                     If p < 2 Then p = 2
    ...
    End If
       Case "END", "ΤΕΛΟΣ"
        ...
        If IsLabelSymbolNew(b$, "ΡΟΥΤΙΝΑΣ", "SUB", Lang) Then
                ...
                Execute = 2
                Exit Function
        ElseIf IsLabelSymbolNew(b$, "ΓΙΑ", "FOR", Lang) Then
         Once = False
                b$ = "NEXT"
        Execute = 2
                Exit Function
        ElseIf IsLabelSymbolNew(b$, "ΕΚΤΥΠΩΣΗΣ", "PRINTING", Lang) Then
        If bstack.toprinter Then
                    pnum = 0
                    oprinter.ClearUp
...
End If
       ...
       End If
        GoTo again3
        Else
         MyEr "Unkown Weak " & w$, "Άγνωστη ισχνή " & w$
        End If
        End If
         If MaybeIsSymbol(b$, "=<") Then
            If FastSymbol(b$, "<=") Then
...
If what$ = "" Then
Identifier = IsLabel(basestack, rest$, what$, True)
...
Exit Function
Case "ICON", "ΕΙΚΟΝΙΔΙΟ"
' ΝΑ ΤΟ ΦΤΙΑΞΩ ΟΠΩΣ ΠΡΕΠΕΙ ΓΙΑ INLINE ΕΙΣΑΓΩΓΗ ΕΙΚΟΝΙΔΙΟΥ...hex$
If ttl Then
If IsStrExp(basestack, rest$, s$) Then
...
Exit Function
Case "WRITE", "ΓΡΑΨΕ"
If IsLabelSymbolNew(rest$, "HEX", "ΔΕΚΑΕΞ", Lang) Then it = 1
If FastSymbol(rest$, "#") Then
...
Exit Function
Case "ΕΚΤΕΛΕΣΗ", "EXECUTE"
CommExecAndTimeOut basestack, rest$
' ΝΕΑ ΛΙΣΤΑ
Case "ΑΝΑΚΤΗΣΗ", "RETRIEVE"
getrow basestack, rest$, , , Lang
Exit Function
Case "ΑΝΑΖΗΤΗΣΗ", "SEARCH"
getrow basestack, rest$, , "", Lang
Exit Function
Case "ΠΡΟΣΘΗΚΗ", "APPEND"
...
ElseIf what$ = "ΚΙΝΗΣΗ" And IsLabelSymbol(rest$, "ΚΕΝΤΡΟ") Then
x = interpret(basestack, "MODE" & Str$(players(GetCode(scr)).SZ) & "," & Str$(scr.Width) & "," & Str$(scr.Height))
...
If IsLabelSymbolNew(rest$, "ΦΟΡΤΩΣΕ", "LOAD", Lang) Then
    ...
    ElseIf AVIUP Then
        ...
        ElseIf IsLabelSymbolNew(rest$, "ΚΡΥΨΕ", "HIDE", Lang) Then
                ...
                Identifier = True
                Exit Function
        ElseIf IsLabelSymbolNew(rest$, "ΚΡΑΤΗΣΕ", "PAUSE", Lang) Then
                If MediaPlayer1.isMoviePlaying Then MediaPlayer1.pauseMovie
                Identifier = True
                Exit Function
        ElseIf IsLabelSymbolNew(rest$, "ΠΑΙΞΕ", "PLAY", Lang) Then
                ...
                Identifier = True
                Exit Function
        ElseIf IsLabelSymbolNew(rest$, "ΞΕΚΙΝΑ", "RESTART", Lang) Then
...
x1 = Abs(IsLabel(basestack, rest$, SS$, , w$))
If x1 = 1 Then
SS$ = w$
...
Case "LOAD", "ΦΟΡΤΩΣΕ" '
Do
x1 = Abs(IsLabel(basestack, rest$, s$, , w$))
If x1 = 1 Then
s$ = w$
Else
rest$ = s$ + rest$
...
If (Asc(SS$) > 127 And myUcase(Left$(SS$, 5)) <> "ΚΛΑΣΗ" And myUcase(Left$(SS$, 5)) <> "ΤΜΗΜΑ" And myUcase(Left$(SS$, 9)) <> "ΣΥΝΑΡΤΗΣΗ") Or (((AscW(SS$) And &H4000) = &H4000)) Then
    SS$ = mycoder.must(SS$)
    If NORUN1 Then
        Clipboard.clear
...
ElseIf IsLabelSymbolNew(rest$, "ΣΤΟ", "TO", Lang, True) Then
...
Do
    ...
    it = Abs(IsLabel(basestack, rest$, w$))
    If MaybeIsSymbol(rest$, ")") Then rest$ = "0" + rest$
...
Else
sHelp "Μ2000 [HELP]", "Write END for exit from this program" & vbCrLf & "See ALL commands  (click on ALL)" & vbCrLf & "George Karras 2014", (ScrX() - 1) * 3 / 5, (ScrY() - 1) * 1 / 7
...
Case "SOUNDREC", "ΗΧΟΓΡΑΦΗΣΗ"  '*************************************************SOUND RECORDING
' not tested yet...
    ...
    If IsLabelSymbolNew(rest$, "ΕΙΣΑΓΩΓΗ", "INSERT", Lang) Then
        sRec.Capture True
    ElseIf IsLabelSymbolNew(rest$, "ΑΛΛΑΓΗ", "OVERWRITE", Lang) Then
        sRec.ReCapture
    ElseIf IsLabelSymbolNew(rest$, "ΑΠΟΚΟΠΗ", "DELETE", Lang) Then
    If IsExp(bstack, rest$, x) Then
    Else
    x = 0
    End If
    ...
    ElseIf IsLabelSymbolNew(rest$, "STOP", "ΔΙΑΚΟΠΗ", Lang) Then
        sRec.recStop
    ElseIf IsLabelSymbolNew(rest$, "ΔΟΚΙΜΗ", "TEST", Lang) Then
        sRec.recPlay
    ElseIf IsLabelSymbolNew(rest$, "ΘΕΣΗ", "POS", Lang) Then
        ...
        ' SEEK
            ...
            End If
        End If
    ElseIf IsLabelSymbolNew(rest$, "ΣΩΣΕ", "SAVE", Lang) Then
        If IsStrExp(bstack, rest$, s$) Then
            sRec.SaveAs s$
        Else
            sRec.Save
        End If
    ElseIf IsLabelSymbolNew(rest$, "ΚΛΕΙΣΕ", "END", Lang) Then
        Set sRec = Nothing
    End If
    Else
...
Case "KEYBOARD", "ΠΛΗΚΤΡΟΛΟΓΙΟ"   '***********************************************************KEYBOARD ! OPEN osk.exe
...
Case "ΝΗΜΑ", "THREAD"  ' ************************************************************************* THREAD CONSTRUCT AND HANDLE FROM OTHER POINT
      If FastSymbol(rest$, "{") Then
        ...
        If FastSymbol(rest$, "}") Then
                ...
                Case 1
                        ...
                        Set bs = Nothing
                        If IsLabelSymbolNew(rest$, "ΚΑΘΕ", "INTERVAL", Lang) Then GoTo chekInterval
                Case Else
                       MissingnumVar
                        ifier = True
                        I2 = False
...
ElseIf IsExp(bstack, rest$, p) Then
    ...
    ElseIf IsLabelSymbolNew(rest$, "ΚΡΑΤΑ", "HOLD", Lang) Then
        TaskMaster.Message CLng(p), 0
    ElseIf IsLabelSymbolNew(rest$, "ΞΕΚΙΝΑ", "RESTART", Lang) Then
        TaskMaster.Message CLng(p), 2
    ElseIf IsLabelSymbolNew(rest$, "ΚΑΘΕ", "INTERVAL", Lang) Then
chekInterval:
        If IsExp(bstack, rest$, x) Then
            If x < 2 Then x = 2
...
If ML <> 1 Then
    Do
        ...
        stac1$ = stac1$ & Str$(p)
        ElseIf IsStrExp(bstack, rest$, s$) Then
        ' να το αλλάξω σε stack κανονικό...
        stac1$ = stac1$ & Sput(s$)
        Else
        Exit Do
        End If
        If Not FastSymbol(rest$, ",") Then Exit Do
        'να το αλλάξω σε stack κανονικό...να φύγει το κόμμα
        'stac1$ = stac1$ & ", "
    Loop
...
If IsSupervisor Then
On Error Resume Next
x1 = Abs(IsLabel(bstack, rest$, s$, , w$))
If x1 = 1 Then
s$ = w$
Else
rest$ = s$ + rest$
...
Case "CLIPBOARD", "ΠΡΟΧΕΙΡΟ"   '******************************************************PUT TO  CLIPBOARD STRING /OR STRING WITH IMAGE INSIDE
    If IsStrExp(bstack, rest$, s$) Then
...
vH_title$ = ""
    ...
    fHelp bstack, s$, Asc(s$ + Mid$(" Σ", Abs(pagio$ = "GREEK") + 1)) < 128
...
what$ = myUcase$(what$)
    If x1 = 1 Then   '' Or X1 = 3 Then  '' not yet for string
        If x1 = 1 Then
            If GetVar(bstack, what$, i) Then
               If IsLabelSymbolNew(rest$, "ΤΙΠΟΤΑ", "NOTHING", Lang) Then
            ...
            End If
         End If
        If IsLabelSymbolNew(rest$, "ΑΠΟ", "LIB", Lang) Then
...
Case "ABOUT", "ΠΕΡΙ"
        ...
        Exit Function
Case "OPTIMIZATION", "ΒΕΛΤΙΣΤΟΠΟΙΗΣΗ"
...
Else
PlainBaSket scr, players(prive), "ΧΩΡΙΣ ΟΡΙΟ ΑΝΑΔΡΟΜΗΣ ΣΤΙΣ ΣΥΝΑΡΤΗΣΕΙΣ"
End If
Else
...
Else
PlainBaSket scr, players(prive), "Γιώργος Καρράς (C), Καναλάκι, Ελλάδα 1999-2014"
End If
crNew bstack, players(prive)
Case "FILES", "ΑΡΧΕΙΑ"
Set photo = New recDir
...
Case "CIRCLE", "ΚΥΚΛΟΣ" ' LINE X1,Y1,COL
par = False
...
If FastSymbol(rest$, "#") Then
    ...
    ifier = True
    If Uni(f) Then
    If Not getUniStringlINE(f, s$) Then ifier = False: MyEr "Can't input, not UTF16LE", "Δεν μπορώ να εισάγω, όχι UTF16LE": Exit Function
    Else
...
crNew bstack, players(prive)
Exit Function
Case "FONT", "ΓΡΑΜΜΑΤΟΣΕΙΡΑ"     '********************************************************FONT "TAHOMA"
    ' NAME ONLY
    ' RESET WINDOW
    prive = GetCode(scr)
    If IsStrExp(bstack, rest$, s$) Then
...
ifier = False
prive = GetCode(scr)
''If Lang Then pa$ = "LINE" Else pa$ = "ΓΡΑΜΜΗ"
If IsExp(bstack, rest$, p) Then
If Not FastSymbol(rest$, ",") Then MissPar:: Exit Function
Else
p = 0
...
For i = 1 To 13
If FK$(i) <> "" Then
pa$ = pa$ + placeme$("ΚΛΕΙΔΙ", "FKEY", Lang) + Right$(" " & Str$(i), 3) & " [" & FK$(i) & "]" ' FKEY
...
If x1 = 1 Then
s$ = w$
 If Not IsLabelSymbolNew(rest$, "ΩΣ", "AS", Lang) Then ifier = False: Exit Function
 y1 = Abs(IsLabel(bstack, rest$, SS$, , w$))
 If y1 = 0 Then
rest$ = w$ + rest$
...
If Not IsStrExp(bstack, rest$, s$) Then
    If Not Abs(IsLabel(bstack, rest$, s$, , w$)) = 1 Then
    If NOEDIT Then
   ...
   If HERE$ <> "" Then
             ...
             If Lang = 1 Then
                rest$ = "GLOBAL " + Chr(34) + HERE$ + "." + bstack.GroupName + f$ + Chr(34) + " { " + SS$ + "} " + rest$
                  Else
                  rest$ = "ΓΕΝΙΚΗ " + Chr(34) + HERE$ + "." + bstack.GroupName + f$ + Chr(34) + " { " + SS$ + "} " + rest$ ''SBgroup(i) +  " { " + ss$ + "} " + Rest$
                  End If
   GoTo BYPASS3
   Else
...
Case "DOCUMENT", "ΕΓΓΡΑΦΟ", "DIM", "ΠΙΝΑΚΑΣ", "GROUP", "ΟΜΑΔΑ", "LONG", "ΜΑΚΡΥΣ"
...
Else
 GlobalGroup = Identifier(basestack, "@ΟΜΑΔΑ", rest$)
End If
HERE$ = ohere$
